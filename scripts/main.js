...
(function initRegionUI() {
  ...
  function applyCollapse() {
    allBtns.forEach((btn, i) => {
      if (!query) {
        // Режим коллапса: показываем первые VISIBLE, остальные по флагу
        btn.style.display = (i >= VISIBLE && !isExpanded) ? "none" : "";
      }
    });
  }
  ...
  if (rsInput) {
    rsInput.addEventListener("input", () => {
      query = rsInput.value.trim();
      if (rsClear) rsClear.hidden = !query;
      applySearch();
    });
  }
