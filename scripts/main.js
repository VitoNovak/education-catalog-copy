...
const VISIBLE_REGIONS = 8; // сколько регионов показывать по умолчанию
...
// =============================================
//  ЛОГИКА ПОИСКА ПО РЕГИОНАМ И "ПОКАЗАТЬ ВСЕ"
// =============================================
...
(function initRegionUI() {
  ...
  if (!regionList || !toggleBtn) return;

  const allBtns = Array.from(regionList.querySelectorAll(".region-button"));
  const totalHidden = allBtns.length - VISIBLE_REGIONS;

  let isExpanded = false;
  let searchQuery = "";

  // Помечаем кнопки, которые скрыты по умолчанию
  allBtns.forEach((btn, i) => {
    if (i >= VISIBLE_REGIONS) btn.classList.add("region-hidden");
  });

  // Счётчик на кнопке "показать все"
  if (totalHidden > 0) {
    countBadge.textContent = "ещё " + totalHidden;
    regionList.classList.add("collapsed");
  } else {
    toggleWrap.hidden = true;
  }

  // Переключить показать все / скрыть
  toggleBtn.addEventListener("click", () => {
    isExpanded = !isExpanded;
    toggleBtn.setAttribute("aria-expanded", isExpanded);
    if (isExpanded) {
      regionList.classList.remove("collapsed");
      toggleBtn.querySelector(".region-toggle-count").textContent = "";
      toggleBtn.childNodes[0].textContent = "Скрыть ";
    } else {
      regionList.classList.add("collapsed");
      countBadge.textContent = "ещё " + totalHidden;
      toggleBtn.childNodes[0].textContent = "Показать все регионы ";
    }
    // Сбрасываем поиск при сворачивании
    if (!isExpanded && regionSearchInput) {
  ...
  function applyRegionSearch() {
    ...
    allBtns.forEach(btn => {
      ...
      if (q) {
        // При поиске показываем все совпадающие, скрываем остальные
        btn.style.display = match ? "" : "none";
        if (match) visibleCount++;
      } else {
        // Без поиска возвращаем обычный режим
        btn.style.display = "";
      }
    });

    // Показываем заглушку если ничего не найдено
    noResults.hidden = !(q && visibleCount === 0);

    // Прячем кнопку "показать все" во время поиска
    if (q) {
      toggleWrap.hidden = true;
      regionList.classList.remove("collapsed");
    } else {
      toggleWrap.hidden = totalHidden <= 0;
      if (!isExpanded) regionList.classList.add("collapsed");
      else regionList.classList.remove("collapsed");
    }
  }

  if (regionSearchInput) {
    regionSearchInput.addEventListener("input", () => {
      searchQuery = regionSearchInput.value.trim();
      regionSearchClear.hidden = !searchQuery;
      applyRegionSearch();
    });
  }
